...
async function tips(ckArr) {
    ...
    await getVersion();
    await getVersion();
	DoubleLoglog(`\n============ 📌 当前版本：V${scriptVersion}  最新版本：V${scriptVersionLatest} ============`)
...
async function start() {
	...
	if (ck_status) {
		...
		await openbox('开宝箱');
		await $.wait(3 * 1000);
		await ad_platform('看视频广告');
		
	if (local_hours() == 20) {
	    await GetAdVideoBigReward('广告累计奖励');
	    await $.wait(3 * 1000);
	    await getBubbleTaskReward('气泡任务');
		
		}
		await $.wait(3 * 1000);
		await myWelfareNew('资产查询');
	}
}
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `http://git.lihui.ml/https://raw.githubusercontent.com/meetclover/JavaScript/main/${alias_name}.js`,
        }
